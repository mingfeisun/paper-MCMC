...
This optimization process is just the actor-critic mechanism in reinforcement learning~\citep{konda2000actor}. \sw{This is really vague: how exactly is it like AC?  Your claim is that $\Psi$ is like the critic?  But you still have to learn $f$ by optimising $\omega$, no?  That seems fundamentally different from AC where the reward is given, not learned.}
