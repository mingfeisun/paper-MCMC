...
\vit{I do not think you named $d_{\pi}$ state occupancy before. Do this when you introduce it as well.} For any state occupancy $d_{\pi_{\theta}}$, in either the average or discounted cases, and any function $f_{\omega}$, 
\begin{equation*}
...
\end{theorem}
\vit{You often use 'A is defined according to B'. It is not clear what it means.}\vit{What is $\phi(s,a,)$?}
...
\vit{Use use $\rho$ in the definition of the MDP. Maybe we should change it there.}
...
\end{equation*}
where $\gamma\in[0, 1]$ is a discount rate ($\gamma=1$ is allowed only in episodic tasks). \vit{You do not have 0 in the discounting coeff range in the definition. Probably you want to exclude 1 either, because the horison is infinite.}
\end{proof}
